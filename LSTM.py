...
def init_Model():
    ...
    model.add(layers.Conv1D(32, 4, activation='leaky_relu', padding='same', input_shape=(128, 128)))
    model.add(layers.LSTM(32, return_sequences=True))
    model.add(layers.MaxPooling1D(2))
    model.add(layers.Conv1D(16, 8, activation="leaky_relu", padding='same'))
...
[tot_train_dim1, tot_train_dim2, tot_train_dim3] = total_train.shape
pca = PCA(n_components=100)
pca.fit(np.reshape(total_train, (tot_train_dim1, tot_train_dim2*tot_train_dim3)))
total_train = np.reshape(total_train, (tot_train_dim1, tot_train_dim2, tot_train_dim3))

[tot_test_dim1, tot_test_dim2, tot_test_dim3] = total_test.shape
pca = PCA(n_components=100)
pca.fit(np.reshape(total_test, (tot_test_dim1, tot_test_dim2*tot_test_dim3)))
total_test = np.reshape(total_test, (tot_test_dim1, tot_test_dim2, tot_test_dim3))
...
predictions = []
for k in total_test:
    predictions.append(np.argmax(model.predict(np.reshape(k, (1, 128, 128)))))
...
print('*' * 100)
[test_dim_1, test_dim_2, test_dim_3] = total_test.shape
samples = np.random.randint(0, test_dim_1, 15)

for k in samples:
    print('-' * 30)
    print("Sample: " + str(k))
    prediction = np.argmax(model.predict(np.reshape(total_test[k], (1, 128, 128))))
    if prediction == 0:
